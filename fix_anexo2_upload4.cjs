...
if (index !== -1) {
  content = content.substring(0, index) + replacement + content.substring(index + targetStr.length);
  fs.writeFileSync('src/components/EvaluacionDocentePanel.tsx', content, 'utf8');
  console.log("Successfully removed Soporte Adjunto.");
} else {
  console.log("String not found! The target string doesn't exactly match.");
}
