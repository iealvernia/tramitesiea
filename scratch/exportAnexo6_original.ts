...
export const generarAnexo6Excel = (evalDoc: Evaluacion1278, teacher: DocenteEvaluacion, institutionName: string) => {
  ...
  for (let R = 0; R < wsData.length; ++R) {
    for (let C = 0; C < COLS; ++C) {
      ...
      else if (R >= 2 && R <= 9 && (C === 3 || C === 6 || C === 13 || C === 16 || C === 17)) {
          if (valStr !== "No." && valStr !== "CC" && valStr !== "Nombres y apellidos") style = { ...YELLOW_CELL };
      }
      else if (R >= 10 && R <= 12 && (C === 2 || C === 6 || C === 12 || C === 15)) {
          style = { ...YELLOW_CELL };
      }
      ...
      if (R === rDiasTotal && C === 16) style = { ...RED_BG, alignment: { horizontal: 'center', vertical: 'center' }, border: BORDER_ALL };
      ...
      if (vStr.includes("Competencias objeto de mejoramiento")) {
          if (R === r - 3 && C >= 0 && C <= 8) style = { ...GREEN_CELL };
          if (R === r - 3 && C >= 9 && C <= 17) style = { ...GREEN_CELL };
      }
      if (R === r - 2 && C >= 0 && C <= 8) style = { ...GREEN_CELL };
      if (R === r - 2 && C >= 9 && C <= 17) style = { ...GREEN_CELL };
      ...
      if (C >= 14 && C <= 17 && (valStr === "0.0" || (!isNaN(parseFloat(valStr)) && parseFloat(valStr).toString() === valStr || valStr.match(/^\d+\.\d+$/)))) {
