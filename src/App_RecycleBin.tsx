...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'novedades' | 'empleados' | 'reportes' | 'computo' | 'matriculas' | 'certificados' | 'certificadosPama' | 'constancias' | 'configuracion'>('novedades');
  ...
  const handleClearAllData = async () => {
    setEmployees([]);
    setNovedades([]);
    localStorage.removeItem('alvernia_employees');
    localStorage.removeItem('alvernia_novedades');
    ...
    try {
      ...
      console.warn('Could not clear remote Supabase DB records:', e);
    }

    showToast("¡Datos borrados correctamente! Todos los registros se han limpiado. Ahora puede registrar o importar nuevos datos reales.");
  };
  ...
  const handleAddEmployeeSubmit = async (e: React.FormEvent) => {
    ...
    // Check duplication
    if (employees.some(emp => emp.cedula === newEmpCedula)) {
    ...
    <div className="flex h-screen w-full bg-[#f8fafc] overflow-hidden font-sans antialiased text-slate-800" id="main-app-container">
      ...
      <aside className="w-64 bg-slate-900 flex flex-col border-r border-slate-800 shrink-0 h-full hidden lg:flex" id="sleek-sidebar">
        ...
        <nav className="flex-1 p-4 space-y-1" id="sidebar-tab-navigation">
          ...
          {/* Academic / Student Modules Division */}
          <div className="pt-4 pb-1 group-data-[collapsed=true]:hidden">
            <p className="px-4 text-[9px] font-extrabold text-slate-500 uppercase tracking-widest leading-none">
              Gestión Alvernia (Supabase)
            </p>
          </div>

          <button
            onClick={() => setActiveTab('matriculas')}
            className={`w-full px-4 py-2.5 rounded-lg cursor-pointer transition-all flex items-center gap-3 font-semibold text-xs text-left uppercase tracking-wider ${
              activeTab === 'matriculas'
                ? 'bg-emerald-655 bg-emerald-600 text-white shadow-md shadow-emerald-600/10'
                : 'text-slate-400 hover:text-white hover:bg-slate-800'
            }`}
          ...
          >
            <FileCheck className="w-4 h-4 text-emerald-400 shrink-0" />
            Constancias Matrícula
          </button>

          {/* Core Settings utility tab */}
          <div className="pt-4 pb-1 group-data-[collapsed=true]:hidden">
            <p className="px-4 text-[9px] font-extrabold text-slate-500 uppercase tracking-widest leading-none">
              Ajustes Avanzados
            </p>
          ...
          >
            <Settings className="w-4 h-4 text-blue-400 shrink-0 animate-spin-slow" />
            Configuración Sistema
          </button>
        </nav>

        {/* System utility section */}
        <div className="px-4 py-2 border-t border-slate-800 my-2">
          <button
            onClick={() => setConfirmResetOpen(true)}
            className="w-full px-4 py-2.5 text-rose-400 hover:text-white hover:bg-rose-950/40 bg-rose-950/10 rounded-lg cursor-pointer transition-all flex items-center gap-3 font-semibold text-xs text-left uppercase tracking-wider border border-rose-900/30"
            id="sidebar-btn-reset-data"
            title="Borrar todos los datos cargados para ingresar información real"
          >
        ...
        <div className="p-4 border-t border-slate-800" id="sidebar-profile-footer">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden" id="workspace-main-panel">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6" id="workspace-dynamic-content">
          ...
          {/* Supabase Core Sync Status Warning & SQL Helper */}
          {['novedades', 'empleados', 'computo', 'reportes'].includes(activeTab) && (
            <div className="space-y-4" id="supabase-sync-indicator-container">
              {supabaseSyncStatus === 'error' && (
                <div className="bg-rose-50/90 border border-rose-150 p-6 rounded-2xl space-y-4 shadow-sm animate-fade-in" id="supabase-error-banner">
                  <div className="flex items-start gap-3.5">
                    <div className="p-2.5 bg-rose-650 text-white rounded-xl shrink-0 shadow-md shadow-rose-655/10">
                      <Settings className="w-5 h-5 shrink-0" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-black text-rose-900 uppercase tracking-wider leading-none">
                        ⚙️ Acción Requerida: Crear las tablas de Permisos en Supabase
                      </h4>
                      <p className="text-xs text-rose-700 font-medium mt-2 leading-relaxed">
                        El sistema offline de Alvernia está activo mediante caché local (localStorage), pero para guardar la información persistentemente en la nube de Supabase necesitas crear las tablas <code className="bg-rose-100 px-1 py-0.5 rounded font-mono font-bold text-rose-800">employees</code> y <code className="bg-rose-100 px-1 py-0.5 rounded font-mono font-bold text-rose-800">novedades</code>.
                      </p>
                      <p className="text-[10px] text-rose-600/90 font-mono mt-1 font-bold">
                        Detalle técnico: "{supabaseDbError}"
                      </p>
                    </div>
                    <button 
                      onClick={() => fetchFromSupabase()} 
                      className="px-3 py-1.5 bg-rose-200/50 hover:bg-rose-200 text-rose-850 text-[10px] font-black rounded-lg cursor-pointer transition-all uppercase tracking-wider shrink-0"
                    >
                      Re-intentar Conexión
                    </button>
                  </div>
                  
                  <div className="bg-slate-900 rounded-xl p-4 border border-slate-800 relative group overflow-hidden" id="supabase-sql-instructions">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider font-mono">
                        Script de Configuración SQL para la sección de Permisos
                      </span>
                      <span className="text-[9px] bg-slate-800 text-slate-300 py-0.5 px-2 rounded-full font-semibold font-mono">
                        Copiar en SQL Editor de Supabase
                      </span>
                    </div>
                    <pre className="text-xs text-blue-300 overflow-x-auto font-mono whitespace-pre select-all max-h-48 p-1 scrollbar-thin">
{`-- 1. Crear Tabla de Empleados (Personal de I.E. Alvernia)
CREATE TABLE IF NOT EXISTS public.employees (
    id TEXT PRIMARY KEY,
    nombre TEXT NOT NULL,
    cedula TEXT NOT NULL UNIQUE,
    cargo TEXT NOT NULL,
    sede_trabajo TEXT NOT NULL,
    dificil_acceso TEXT NOT NULL,
    horas_aula INTEGER DEFAULT 0,
    horas_libres INTEGER DEFAULT 0,
    area_desempeno TEXT NOT NULL,
    tipo_nombramiento TEXT NOT NULL,
    activo BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 2. Crear Tabla de Novedades y Permisos
CREATE TABLE IF NOT EXISTS public.novedades (
    id TEXT PRIMARY KEY,
    empleado_id TEXT NOT NULL,
    clase_novedad TEXT NOT NULL,
    sede_novedad TEXT NOT NULL,
    fecha_inicio TEXT NOT NULL,
    fecha_fin TEXT NOT NULL,
    esta_laborando_normalmente TEXT NOT NULL,
    se_le_asigno_carga_academica TEXT NOT NULL,
...
-- 3. Habilitar Seguridad RLS y Políticas de acceso libre para testing
ALTER TABLE public.employees ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public Select Employees" ON public.employees;
...
DROP POLICY IF EXISTS "Public Select Novedades" ON public.novedades;
DROP POLICY IF EXISTS "Public Insert Novedades" ON public.novedades;
DROP POLICY IF EXISTS "Public Update Novedades" ON public.novedades;
DROP POLICY IF EXISTS "Public Delete Novedades" ON public.novedades;
CREATE POLICY "Public Select Novedades" ON public.novedades FOR SELECT USING (true);
CREATE POLICY "Public Insert Novedades" ON public.novedades FOR INSERT WITH CHECK (true);
CREATE POLICY "Public Update Novedades" ON public.novedades FOR UPDATE USING (true) WITH CHECK (true);
CREATE POLICY "Public Delete Novedades" ON public.novedades FOR DELETE USING (true);`}
                    </pre>
                  </div>
                </div>
              )}
              ...
              {supabaseSyncStatus === 'synced' && (
                <div className="bg-emerald-50/80 border border-emerald-100 p-4 rounded-xl flex items-center justify-between gap-3 shadow-xs" id="supabase-success-pill">
                  <div className="flex items-center gap-2.5">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0"></span>
                    <p className="text-xs text-emerald-800 font-semibold uppercase tracking-wider">
                      ☁️ Módulo de Permisos Conectado y Sincronizado con Supabase
                    </p>
                  </div>
                  <span className="text-[10px] bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">
                    Nube Activa (Persistente)
                  </span>
                </div>
              )}

              {supabaseSyncStatus === 'syncing' && (
                <div className="bg-blue-50/80 border border-blue-100 p-4 rounded-xl flex items-center justify-between gap-3 shadow-xs" id="supabase-syncing-pill">
                  <div className="flex items-center gap-2.5">
                    ...
                    <p className="text-xs text-blue-800 font-semibold uppercase tracking-wider">
                      Sincronizando módulo de permisos con Supabase...
                    </p>
                  </div>
                </div>
              )}
          ...
          {/* KPI Cards section */}
          {!['matriculas', 'certificados', 'certificadosPama', 'constancias', 'configuracion'].includes(activeTab) && (
          ...
          {/* ==================== TAB 9: CONFIGURACIÓN GENERAL DEL SISTEMA ==================== */}
          {activeTab === 'configuracion' && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-4" id="reset-confirm-backdrop">
            ...
            >
              ...
              <div className="bg-red-600 px-6 py-4 flex items-center gap-3 text-white">
                ...
                <h3 className="font-bold text-base leading-none">Confirmación Crítica de Seguridad</h3>
              </div>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-2">
                  <p className="text-slate-800 text-sm font-bold">
                    ¿Está absolutamente seguro de que desea limpiar la base de datos de la aplicación?
                  </p>
                  <p className="text-slate-500 text-xs leading-relaxed">
                    Esta acción eliminará de forma instantánea y definitiva todos los registros locales de:
                  </p>
                  <ul className="list-disc list-inside text-xs font-bold text-slate-600 pl-2 space-y-1">
                    <li>Planilla General de Talento Humano ({employees.length} funcionarios)</li>
                    <li>Historial Completo de Permisos y Novedades ({novedades.length} registros)</li>
                  </ul>
                </div>
                ...
                <div className="bg-amber-50 text-amber-950 p-3.5 rounded-xl border border-amber-200 text-[11px] leading-relaxed flex gap-2">
                  ...
                  <div>
                    ...
                    Esto borrará los datos de demostración pre-cargados para que pueda cargar, mediante importación de Excel o registro directo, sus <strong>datos reales institucionales</strong>.
                  </div>
                </div>
              ...
              <div className="bg-slate-50 px-6 py-4 flex items-center justify-end gap-3 border-t border-slate-100">
                ...
                <button
                  ...
                  className="py-2 px-4 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-colors cursor-pointer font-bold text-xs shadow-sm shadow-red-500/10 flex items-center gap-1.5"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  Sí, Borrar Todo y Cargar Datos Reales
                </button>
              </div>
            </motion.div>
          </div>
