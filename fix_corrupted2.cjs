...
const regex = /<AlertCircle className=\{\`w-4 h-4 shrink-0 \$\{teacherMessagesStatus[^>]+>\s+activeTab === 'reportes'[\s\S]*?id="tab-btn-reportes-sidebar"\s*>\s*<FileSpreadsheet className="w-4 h-4 shrink-0" \/>\s*Reportes Generales\s*<\/button>/m;
