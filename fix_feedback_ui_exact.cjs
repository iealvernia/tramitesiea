...
const startMarker = '              {/* Feedback Inputs */}';
const endMarker = '              </div>\n            </motion.div>\n          </div>\n        )}\n      </AnimatePresence>\n\n      {/* --- NOTIFICATIONS --- */}';

if (c.includes(startMarker) && c.includes(endMarker)) {
  const before = c.split(startMarker)[0];
  const after = endMarker + c.split(endMarker)[1];
...
`;
  ...
  fs.writeFileSync('src/components/EvaluacionDocentePanel.tsx', before + newCode + after, 'utf8');
