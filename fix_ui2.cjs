...
// Remove the Soporte Adjunto upload UI from Anexo 2
const fileUploadRegex = /<div className="md:col-span-8">([\s\S]*?)<\/div>\s*\{\/\* Attachment management \*\/}[\s\S]*?<\/div>/;
content = content.replace(fileUploadRegex, '<div className="md:col-span-12">$1</div>');

fs.writeFileSync('src/components/EvaluacionDocentePanel.tsx', content, 'utf8');
console.log('Restored UI fixes and removed Anexo 2 file upload block');
